...
def log_memory(logger, stage):
    ...
    msg = f'[mem] {stage} | proc_rss={rss_gb:.2f} GiB | sys_used={vm.used / 1024 / 1024 / 1024:.2f} GiB | sys_avail={vm.available / 1024 / 1024 / 1024:.2f} GiB'
    if torch.cuda.is_available():
        msg += f" | cuda_alloc={torch.cuda.memory_allocated() / 1024 / 1024:.1f} MiB | cuda_reserved={torch.cuda.memory_reserved() / 1024 / 1024:.1f} MiB"
    logger.log(msg)
...
def main(args):
    ...
    log_root = os.path.join(script_dir, 'prune_log')
    ...
    logger.log('Use SAAP pruner...')
    logger.log('Protected layers = {}'.format(sorted(protected_layers)))
    logger.log('Pruning Attention Layer = {}'.format(attn_layers))
    logger.log('Pruning MLP Layer = {}'.format(mlp_layers))
    ...
    if bool(getattr(args, 'save_model', False)):
        ...
        logger.log(f'[post_prune] saved prune-only checkpoint to {prune_only_ckpt_path}')
...
def build_args(base_model='/home/easyai/llm_weights/decapoda-llama-7b-hf', save_ckpt_log_name='llama7b_cfspffnflap_oldstyle20_reconstructonly_bs8', pruning_ratio=0.2):
    ...
    args.block_attention_layer_start = 4
    args.block_attention_layer_end = 32
    args.block_mlp_layer_start = 4
    args.block_mlp_layer_end = 32
    args.protect_first_n_layers = 2
    args.protect_last_layer = True
